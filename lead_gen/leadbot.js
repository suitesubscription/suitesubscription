...
(function () {
  ...
  const EMAILJS_LEAD_TEMPLATE  = 'template_9fd2q2s'; // new leadbot notification template
  ...
  style.textContent = `
    ...
    .ss-input:focus, .ss-select:focus { border-color: #F5E642; }
    .ss-btn {
    ...
    .ss-error { color: #e53e3e; font-size: 0.8rem; display: none; text-align: center; }
    .ss-success { padding: 1.5rem 1.25rem; text-align: center; }
    ...
    .ss-dot { width: 8px; height: 8px; background: #1DB954; border-radius: 50%; display: inline-block; margin-right: 4px; animation: ss-pulse 1.5s infinite; }
  ...
  panel.innerHTML = `
    ...
    <div class="ss-body" id="ss-form-body">
      ...
      <div class="ss-error" id="ss-error">Please fill in your name, email and location.</div>
  ...
  document.getElementById('ss-submit-btn').addEventListener('click', async () => {
    ...
    let sent = false;
    try {
      await window.emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_LEAD_TEMPLATE, {
        lead_name:     name,
        lead_email:    email,
        lead_phone:    phone || 'Not provided',
        lead_location: location + ', GA',
